...
func (s *Server) serveConn(nc net.Conn) {
	...
	reqHeaderBuf := frameconn.Frame{
		Buffer: make([]byte, 1<<15), // FIXME const
	}
	if err := c.ReadFrame(&reqHeaderBuf); err != nil {
		s.log.WithError(err).Error("error reading structured part")
		return
	}

	endpoint := string(reqHeaderBuf.Buffer[0:reqHeaderBuf.BufferLen])

	reqStructured := frameconn.Frame{
		Buffer: make([]byte, 1<<22), // FIXME const
	}
	if err := c.ReadFrame(&reqStructured); err != nil {
		s.log.WithError(err).Error("error reading structured part")
		return
	}
	...
	case EndpointRecv:
		...
		receive := func(w io.Writer) error {
			streamer := streamer{s.log}
			return streamer.readSendStream(c, w)
		}
		res, err = s.h.Receive(ctx, &req, receive) // SHADOWING
	default:
		s.log.WithField("endpoint", endpoint).Error("unknown endpoint")
		return
	}

	resHeaderType := frameconn.ResOkHeader
	if err != nil {
		resHeaderType = frameconn.ResErrHeader
		s.log.WithError(err).Debug("handler returned error")
	}

	resHeaderBuf := frameconn.Frame{
		Type:   resHeaderType,
		Buffer: []byte(err.Error()),
	}
	resHeaderBuf.FitBufferLenToBuffer()
	if err := c.WriteFrame(&resHeaderBuf); err != nil {
	...
	resStructuredBuf := frameconn.Frame{
		Type:   frameconn.Structured,
		Buffer: protobuf,
	}
	resStructuredBuf.FitBufferLenToBuffer()
	if err := c.WriteFrame(&resStructuredBuf); err != nil {
	...
	if sendStream != nil {
		streamer := streamer{s.log}
		streamer.writeSendStream(c, sendStream)
	}
